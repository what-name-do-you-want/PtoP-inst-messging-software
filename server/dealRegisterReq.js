...
function dealRegisterReq(socket,req_to_server,mysql_client){
    console.log("begin dealRegisterReq");
    ...
    // console.log(headpic.length);
    // let new_user = {name:name,password:password,headpic:headpic};
    mysql_client.query("INSERT INTO `userinfo` (name,password,headPic) values(?,?,?)",[name,password,Buffer.from(headpic)],
        function (error, results, fields) {
            ...
            if (error) {
                ...
                socket.write(rsp_buf,(err)=>{
                    if(err){
                        console.log(err);
                        console.log("in dealRegisterReq:send fail");
                    }
                    console.log("dealRegisterReq end");
                });
            }
        }
    );
